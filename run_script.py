...
for i in a_test:
    for e in elbow_test:
        for w in manus_test:
            ...
            theta_0, U_0 = aerofn.trim_aero(W, rho, S_max, elbow, manus, alpha_0, coef_data)

            if U_0 == 0:
                continue

            # Step 2: Solve the eigen problem
            A = dynfn.solve_linsys(m, Iyy, rho, S_max, c_max, elbow, manus, alpha_0, U_0, theta_0, coef_data)
